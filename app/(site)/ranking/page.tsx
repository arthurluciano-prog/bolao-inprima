...
import { calcKOPontos } from '@/lib/ko';

const MEDALS = ['🥇', '🥈', '🥉'];

export default function RankingPage() {
  const [selected, setSelected] = useState<ParticipantStats | null>(null);
  const ranking = getRanking();
  const apurados = jogosApurados();
  const jogosComPontos = selected ? getJogosComPontos(selected.name) : [];
    ...
    <>
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-2 rounded-card border border-gray-200 bg-white p-3 text-xs">
          ...
          <span className="rounded-card bg-gray-100 px-2 py-1 font-medium text-gray-500">
            Pts KO em breve
          </span>
        </div>
        ...
        <div className="flex flex-col gap-2">
          {ranking.slice(0, 3).map((p, i) => {
            const pontosKO = calcKOPontos(p.name);
            const total = p.pontos + pontosKO;
            return (
              <button
                key={p.name}
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-display text-[26px] text-primary">{total}</span>
                </div>
              </button>
            );
          })}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 text-[11px] uppercase tracking-wide text-gray-500">
                ...
                <th className="px-1.5 py-2 text-center">Gr.</th>
                <th className="px-1.5 py-2 text-center">KO</th>
              ...
              {ranking.map((p, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-1.5 py-2.5 text-center text-[13px] text-gray-500">{p.pontos}</td>
                    <td className="px-1.5 py-2.5 text-center text-[13px] text-gray-300">{pontosKO}</td>
          ...
          <div className="fixed inset-x-0 bottom-0 z-40 max-h-[82vh] overflow-y-auto rounded-t-[20px] bg-white p-5 shadow-2xl">
            ...
            <div className="mb-4 flex items-start justify-between">
              <div>
                ...
                <h2 className="font-display text-[30px] leading-none text-primary">
                  {selected.name}
                </h2>
              </div>
              <button
            ...
            {/* Resumo de pontos */}
            {(() => {
              const pontosKO = calcKOPontos(selected.name);
              const total = selected.pontos + pontosKO;
              return (
                <>
                  <div className="mb-2 grid grid-cols-3 gap-2">
                    <div className="rounded-card bg-primary p-3 text-center">
                      <p className="font-display text-[32px] leading-none text-accent">{total}</p>
                      <p className="mt-1 text-[10px] font-semibold uppercase tracking-wide text-white/70">Total</p>
                    </div>
                  ...
                  <p className="mb-5 text-center text-[11px] text-gray-400">
                    Fase de grupos: {selected.pontos} pts · KO: {pontosKO} pts
                  </p>
                </>
              );
            })()}
            ...
            {jogosComPontos.length === 0 ? (
              <div className="rounded-card border border-gray-200 bg-gray-50 p-6 text-center">
                ...
                <p className="text-xs text-gray-400">Os jogos começam em breve!</p>
              </div>
            ) : (
                ...
                {jogosComPontos.map(({ jogo, palpite, resultado, outcome, pontos }) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] text-gray-400">{jogo.grupo} · {jogo.data}</p>
                      <p className="text-[13px] font-semibold text-gray-800">
                        {jogo.time1} {resultado[0]}×{resultado[1]} {jogo.time2}
                      </p>
                      <p className="text-[11px] text-gray-500">
                        Palpite:{' '}
                        <span className="font-semibold">{palpite}</span>
                      </p>
                    </div>
                    <span
